...
for epoch in range(max_iter):
    ...
    for i in range(int(np.floor(len(y_train) / batch_size))):
        ...
        # 更新w、b
        w -= learning_rate / np.square(step) * w_grad
        b -= learning_rate / np.square(step) * b_grad
    ...
    loss_train.append(compute_loss(y_train, y_train_pre, lamda, w) / num_train)

    # 计算验证集的误差和准确率
    y_dev_pre = get_prob(X_dev, w, b)
    acc_validation.append(accuracy(np.round(y_dev_pre), y_dev))
    loss_validation.append(compute_loss(y_dev, y_dev_pre, lamda, w) / num_dev)
# --------------- End --------------- #
...
with open("./data/predict_test.csv", "w+") as csvfile:
